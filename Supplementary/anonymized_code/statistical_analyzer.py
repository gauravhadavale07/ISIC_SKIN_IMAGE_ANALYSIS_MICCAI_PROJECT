...
class StatisticalAnalyzer:
    ...
    def __init__(self, progress_path: Optional[str] = None):
        if progress_path is None:
            progress_path = str(Path(__file__).resolve().parent.parent / "outputs" / "experiment_progress.json")
    ...
    def print_report(self):
        ...
        print(f"\n⚖️ Statistical Significance ({comparison_model} vs Late Fusion)")
        print("-" * 65)
        
        key_metrics = ["AUROC", "F1 (Macro)", "Linear_CKA", "CFR", "Mean_Delta_P"]
        for metric in key_metrics:
            p_test = self.paired_ttest("Late Fusion", comparison_model, metric)
