    ...
    for seed in cfg.seeds:
        ...
        if not os.path.exists(milk10k_csv_path):
            raise FileNotFoundError(
                f"milk10k_train.csv not found at {milk10k_csv_path}. "
                "Place the released split CSV under data_splits/ before running."
            )
                ...
                try:
                    ...
                    if attempt == max_retries:
                        print(f"⚠️ PERMANENT FAILURE for {model_name} (Seed {seed}) after {max_retries} attempts. Skipping.")
                            # Continue with the next architecture/seed.
                    else:
