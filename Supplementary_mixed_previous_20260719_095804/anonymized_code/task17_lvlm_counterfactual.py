...
app = modal.App("lvlm-counterfactual-trap")
...
image = modal.Image.debian_slim().pip_install(
    ...
    ignore=["results", "logs", ".git", "__pycache__"]
)
...
def run_counterfactual_trap():
    ...
    for idx, row in mel_df.iterrows():
        ...
        for cond, text in conditions.items():
            prompt = f"<s>[INST] <image>\nAnalyze this clinical photograph of a patient with this history: [{text}]. What is the diagnosis? Is the diagnosis MEL or NEV? Answer with only the exact abbreviation. [/INST]"
...
def main():
    print("Starting counterfactual trap experiment...")
    results = run_counterfactual_trap.remote()
    
    os.makedirs("results", exist_ok=True)
    with open("results/lvlm_counterfactual_results.json", "w") as f:
    ...
    print("\n" + "="*50)
    print("COUNTERFACTUAL TRAP RESULTS (True Class: MEL)")
    print("="*50)
    ...
    print("Done! Results saved to results/lvlm_counterfactual_results.json")
