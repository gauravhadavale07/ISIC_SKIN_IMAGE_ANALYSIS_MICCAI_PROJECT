...
def build_train_val_loaders(tokenizer, seed):
    ...
    train_sub, _ = random_split(train_ds, [train_size, val_size], generator=gen)
    gen = torch.Generator().manual_seed(seed)
    _, val_sub   = random_split(val_ds,   [train_size, val_size], generator=gen)
    train_loader = DataLoader(train_sub, batch_size=cfg.train.batch_size,
                              shuffle=True, num_workers=2, pin_memory=True)
    val_loader   = DataLoader(val_sub,   batch_size=cfg.train.batch_size,
                              shuffle=False, num_workers=2, pin_memory=True)
