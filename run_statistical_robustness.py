...
def task20_stats():
    ...
    print("="*50)
    if not os.path.exists("results/task20_visual_biopsy_leak.csv"):
        print("File results/task20_visual_biopsy_leak.csv not found.")
        return
        
    df = pd.read_csv("results/task20_visual_biopsy_leak.csv")
...
def task23_stats():
    ...
    df_subset.to_csv("temp_pad_ufes_A.csv", index=False)
    dataset_A = MultimodalDermatologyDataset(csv_file="temp_pad_ufes_A.csv", img_dir=cfg.paths.pad_ufes_img_dir, tokenizer=tokenizer, transform=get_transforms())
    loader_A = DataLoader(dataset_A, batch_size=32, shuffle=False, num_workers=4)

    df_subset['clinical_history'] = profile_B
    df_subset.to_csv("temp_pad_ufes_B.csv", index=False)
    dataset_B = MultimodalDermatologyDataset(csv_file="temp_pad_ufes_B.csv", img_dir=cfg.paths.pad_ufes_img_dir, tokenizer=tokenizer, transform=get_transforms())
    ...
    try:
        os.remove("temp_pad_ufes_A.csv")
        os.remove("temp_pad_ufes_B.csv")
    except:
        pass
